...
A <<host-task-command>> can be either synchronous or asynchronous. The only
difference between a synchronous <<host-task>> and an asynchronous
<<host-task>>, is that the synchronous <<host-task>> guarantees that the {cpp}
callable will have completed before the command submission returns.

A <<host-task>> is enqueued on a <<queue>> via either the
[code]#sync_host_task# or [code]#async_host_task# member functions of the
[code]#handler# class, for the synchronous and asynchronous variants
respectively.
...
A <<host-task,host task>> can be used for two purposes: either as a task which
can perform arbitrary {cpp} code within the scheduling of the <<sycl-runtime>>
or as a task which can perform interoperability at a point within the
scheduling of the <<sycl-runtime>>.

For the first use case, the {cpp} callable doesn't take any parameters and
buffer or image accessors with [code]#target::host_task# or
[code]#image_target::host_task# may be used. These accessors make the buffer or
image available on the host when the callable is executed.

For the second use case, the {cpp} callable takes a reference to an
[code]#interop_handle#. This handle is used for interoperability with
<<native-backend-object,native backend objects>> associated with the
<<host-task-command>> submission. In this case buffer or image accessors with
[code]#target::device#, [code]#target::constant_buffer# or
[code]#image_target::device# can also be captured, however these accessors can
only be used for interoperability through the [code]#interop_handle# interface.
As opposed to the first use case, when using the [code]#interop_handle#, the
<<sycl-runtime>> may not have resolved all of the dependencies when the {cpp}
callable is called, instead some of the dependencies may be accessible as
<<native-backend-object, native backend objects>> through the
[code]#get_native_events# member function of the [code]#interop_handle#. So
when using a <<host-task>> for interoperability, it is the responsibility of
the user to ensure any dependencies exposed through [code]#get_native_events#
are resolved. In addition the [code]#add_events# member function of the
[code]#interop_handle# may be used to propagate to the <<sycl-runtime>> any
dependencies on asynchronous work that may have been started through
interoperability.
...
provided {cpp} callable has returned, and when using interoperability, once all
the event <<native-backend-object, native backend objects>> provided through
[code]#add_events# are complete.
